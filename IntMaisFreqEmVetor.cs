...
namespace ConsoleApp1
{
    internal class IntMaisFreqEmVetor
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"\nO No. mais frequente é {resposta} com {maiorFreq} ocorrências.");
            } 
            else
            {
                Console.WriteLine($"\nOs Nos. mais frequentes são ({resposta}) com {(maiorFreq == 1 ? "1 ocorrência" : maiorFreq.ToString() + " ocorências")} cada.");
            }
        }
    }
}